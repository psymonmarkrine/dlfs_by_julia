...
function Trainer(network, x_train, t_train, x_test, t_test;
    ...
    iter_per_epoch = max(Integer(round(train_size / mini_batch_size)), 1)
...
function train_step(self::Trainer)
    ...
    if (self.current_iter % self.iter_per_epoch) == 1
        ...
        if !isnothing(self.evaluate_sample_num_per_epoch)
            ...
            x_train_sample, t_train_sample = self.x_train[1:t,:], self.t_train[1:t]
            x_test_sample, t_test_sample = self.x_test[1:t,:], self.t_test[1:t]
        end
